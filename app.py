...
이 섹션의 목표는 **영어 보는 눈을 키워주는 것**. 분량보다 *선별*과 *깊이*가 핵심이야.

**🎯 문장 선정 기준 (이게 이 섹션의 생명)**
원문에서 학습 가치가 가장 높은 문장 **2~3개만** 신중하게 골라. 그냥 길거나 복잡하다고 고르지 말고, 다음 중 하나에 해당하는 문장만 선정해:
- 한국인이 본능적으로 직역하면 *놓치는 영어 특유의 발상*이 담긴 문장
- 같은 의미를 영어와 한국어가 *다르게 생각한다*는 게 드러나는 문장
- "원어민은 이런 식으로 쓰는구나" 하는 깨달음을 주는 표현이 들어간 문장
- 이 한 문장만 제대로 익혀도 다음에 다른 글을 읽을 때 보는 눈이 달라지는 문장

**🚫 절대 하지 말 것 — 도식적 라벨링 금지**
`[S: I] [V: knew] [O: no one] [수식: in the place]` 같은 태그 분해는 가독성 빵점. 절대 사용 금지.
5형식·8품사 같은 문법책식 분류도 빼. 사람이 옆에서 듣고 싶은 건 *살아있는 설명*이지 *해부 도식*이 아냐.

**📝 각 선정 문장마다 이런 흐름으로 풀어줘**

**▸ 원문**
> 문장을 따옴표로 그대로 인용 (가공/축약 금지)

**▸ 왜 이 문장을 골랐는지 (1~2줄)**
한국인이 이 문장에서 흔히 놓치는 영어의 핵심 감각을 압축해서 미리 짚어주기.

**▸ 친절한 해설 — 자연스러운 한국어 산문으로**
1타 강사가 옆에 앉아 차근차근 풀어주듯, 대화체로 흐르게 써. 다음을 자연스럽게 녹여 넣어:
- 한국어식 직역과 영어식 발상의 결정적 차이는 무엇인지
- 원어민이 *왜 굳이* 이 단어/어순/시제/태를 골랐는지 (선택의 이유)
- 비슷한 표현으로 바꾸면 어감이 어떻게 달라지는지 (예: *I didn't know anyone* vs *I knew no one*)
- 이 문장이 풍기는 분위기·뉘앙스·감정의 결까지

좋은 해설의 예시 톤:
> "한국어로는 '아는 사람이 없었다'고 *없음*을 말하지만, 영어는 'I knew no one'으로 *내가 알았다, 0명을* 이라고 합니다. 동사가 아니라 목적어를 부정하는 거죠. 별것 아닌 차이 같지만 이게 영어를 영어답게 쓰는 핵심 감각이에요. *I didn't know anyone* 도 가능하긴 한데, *I knew no one* 쪽이 훨씬 단호하고 문학적입니다 — '단 한 명도'라는 절대성이 살아나거든요."

**▸ 응용 예문 1~2개**
같은 패턴을 활용한 새 영어 문장 + 한글 해석. 학습자가 바로 입에 붙일 수 있게.

**▸ 핵심 통찰 (한 줄, 굵게)**
이 문장에서 평생 가져갈 영어의 작동 원리를 한 줄로 못 박아주기.
