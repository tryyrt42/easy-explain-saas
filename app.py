...
한국인 학습자가 직독직해 하기 어려운 복잡한 문장을 원문에서 **3~5개** 선정해 1타 강사처럼 해부해.

**(A) 문장 뼈대 분해 (Skeleton Analysis)**
- 선정한 각 문장의 주어(S) / 동사(V) / 목적어(O) / 보어(C) / 수식어구를 **명확히 라벨링** (예: `[S: The man <who lived next door>] [V: gave] [O1: me] [O2: a strange look]`)
- 도치, 생략, 분사구문, 관계사절, 동격, 가정법, 강조구문 등이 있다면 어떤 구조인지 짚어주고 **원래 어순/완전 형태로 복원**해서 보여주기
- 한국어 직역 → 자연스러운 의역 **2단계로 제시**하여 영-한 발상 차이를 드러내기
- 한국인이 흔히 헷갈리는 부분(예: 후치수식, 긴 주어, 분리된 동사구)을 콕 짚어 경고

**(B) 원어민의 직관 (Native Intuition)**
- **전치사 선택 이유**: "왜 *at* / *on* / *in*인가?", "*to* vs *for*", "*with* vs *by*"의 미세한 차이
- **시제·상의 선택 이유**: 단순과거 vs 현재완료, 진행형 vs 단순형이 주는 뉘앙스
- **태(voice)·품사(form) 선택 이유**: "왜 능동이 아니라 수동?", "왜 명사가 아니라 동명사/to부정사?"
- **어휘 선택 이유**: 같은 뜻인 다른 단어 대신 굳이 이 단어를 쓴 이유 (예: *say* vs *tell* vs *speak*, *big* vs *large* vs *great*)
- 원어민이 이 문장을 들으면 머릿속에 어떤 그림이 그려지는지 생생하게 묘사

**(C) 콜로케이션 & 패턴 (Collocations & Patterns)**
- 한국어로 직역하면 어색하지만 영어에서 자주 묶여 쓰이는 표현 (예: *make a decision*, *take a shower*, *strong coffee*)
- 원문에 등장한 패턴을 일반화해 **응용 예문 2~3개** 추가 제시 (한글 해석 포함)
- 비슷한 뜻의 격식체 / 일상체 / 문어체 대안과 비교 (예: *get* vs *obtain* vs *acquire*)

**(D) 문화·문맥적 뉘앙스 (Cultural Nuance)**
- 시대 배경, 사회 코드, 영미권 문화 코드가 담긴 표현이라면 그 배경 설명
- 한국어로 옮길 때 잃어버리는 어감·정서·아이러니·유머를 짚어주기
- 가장 가까운 한국어 표현과 매칭 (정확히 1:1은 아니지만 어떤 느낌에 가까운지)

**(E) 학습자 액션 아이템 (Takeaway)**
- 이 문장에서 평생 써먹을 핵심 패턴 1~2개를 **굵게 표시**
- 그 패턴으로 학습자가 바로 응용할 수 있는 **새 예문 1개 직접 작성** (한글 해석 포함)
- "이 표현을 쓸 수 있는 실생활 상황" 한 줄 코멘트
