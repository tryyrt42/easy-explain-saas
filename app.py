...
# ============================================================
# 📄 개인정보처리방침 (?page=privacy 로 접근 → 고정 URL)
#    - 카카오 비즈 앱 전환 심사용 개인정보처리방침 페이지
#    - 위 전역 CSS가 이미 적용된 뒤라 다크 테마 + Streamlit 크롬 숨김 상태로 표시됨
# ============================================================
def render_privacy_policy():
    st.markdown("""
# Easy-Easy 개인정보처리방침

Easy-Easy(이하 '서비스')는 「개인정보 보호법」을 준수하며, 이용자의 개인정보를
다음과 같이 처리합니다.
...
### 1. 수집하는 개인정보 항목
- **회원 식별 정보**: 이메일 주소 (카카오 또는 구글 소셜 로그인 시 제공받음)
- **서비스 이용 정보**: 이용자가 해석을 위해 업로드한 문서의 내용 (일시적 처리)

### 2. 개인정보의 수집 및 이용 목적
- 이메일: 회원 식별, 로그인 및 계정 관리
- 업로드 문서: 문서 해석 서비스(요약·쉬운 설명) 제공

### 3. 개인정보의 보유 및 이용 기간
- **이메일**: 회원 탈퇴 시까지 보유하며, 탈퇴 즉시 파기합니다.
- **업로드 문서**: 해석 처리를 위해 일시적으로만 사용하며, 처리 완료 후
  캐시가 만료되는 시점에 삭제됩니다. 서버에 영구 저장하지 않습니다.

### 4. 개인정보의 처리위탁 및 제3자 제공
서비스는 원활한 기능 제공을 위해 아래 업체에 개인정보 처리를 위탁합니다.

| 수탁업체 | 위탁 업무 | 위탁 항목 |
|---|---|---|
| Google (Gemini API) | 문서 내용 분석 및 해석 | 업로드 문서 내용 |
| Supabase | 회원 인증 및 회원정보 저장 | 이메일 |

업로드한 문서 내용은 해석을 위해 Google Gemini API로 전송되며,
해석 목적 외에는 사용되지 않습니다.

### 5. 이용자의 권리와 행사 방법
이용자는 언제든지 본인의 개인정보에 대해 열람, 정정, 삭제, 처리정지를
요청할 수 있으며, 서비스 내 회원 탈퇴를 통해 개인정보 삭제를 요청할 수 있습니다.

### 6. 개인정보 보호책임자
- 연락처: **[tryyrt42@gmail.com]**

개인정보 처리에 관한 문의, 불만, 피해구제는 위 연락처로 접수하실 수 있습니다.

### 7. 개인정보처리방침의 변경
본 방침의 내용 추가, 삭제 및 수정이 있을 경우 시행 전 서비스 내 공지를 통해
고지합니다.
""")
...
def render_terms_of_service():
    st.markdown("""
# Easy-Easy 이용약관 (Terms of Service)
...
본 이용약관은 YANG YONGKYU(이하 "운영자")가 제공하는 AI 문서 해석 서비스 **Easy-Easy**(이하 "서비스", easyeasy.kr)의 이용 조건 및 절차, 운영자와 이용자 간의 권리·의무에 관한 사항을 규정합니다.
...
Easy-Easy는 사용자가 업로드한 PDF, Word 등 문서를 AI가 해석·요약하여 쉬운 한국어로 제공하는 웹 기반 SaaS(Software as a Service) 서비스입니다.

서비스는 다음 두 가지 플랜으로 제공됩니다:
...
- 1장(page)은 업로드 문서의 1페이지를 의미합니다.
- FREE 플랜의 사용량은 매월 가입일 기준으로 초기화됩니다.
- PRO 플랜의 사용량은 결제 갱신일 기준으로 초기화됩니다.
...
- 회원가입은 Google, Kakao 소셜 로그인 또는 이메일 인증을 통해 가능합니다.
- 1인 1계정을 원칙으로 하며, 타인 명의로 가입하거나 부정한 목적으로 복수 계정을 생성하는 행위는 금지됩니다.
- 계정 관리 책임은 이용자 본인에게 있으며, 계정 도용으로 인한 피해에 대해 운영자는 책임을 지지 않습니다.

---

### 4. 결제 및 구독

- PRO 플랜 결제는 Lemon Squeezy(Stripe 자회사)를 통해 처리됩니다. Lemon Squeezy는 결제 및 세금 처리의 판매 주체(Merchant of Record)로서 관련 거래를 책임집니다.
- 구독은 결제일 기준으로 매월 자동 갱신됩니다.
- 이용자는 언제든지 구독을 취소할 수 있으며, 취소 시 현재 구독 기간 종료 후 FREE 플랜으로 자동 전환됩니다.
- 결제 관련 문의는 운영자 이메일 또는 Lemon Squeezy 고객센터를 통해 처리됩니다.

---

### 5. 금지 행위

다음 행위는 금지되며, 위반 시 사전 통보 없이 계정이 정지 또는 삭제될 수 있습니다:

- 서비스를 통해 타인의 저작권, 개인정보, 영업 비밀 등을 침해하는 행위
- 자동화된 방법(봇, 스크래핑 등)으로 서비스를 대량 사용하는 행위
- 서비스를 악용하여 허위 정보, 불법 콘텐츠를 생성·유포하는 행위
- 타인의 계정을 도용하거나 서비스 시스템에 무단으로 접근하는 행위
- 그 밖에 관련 법령에 위반되거나 운영자가 불적절하다고 판단하는 행위

---

### 6. 서비스 변경 및 중단

- 운영자는 서비스 내용, 요금, 플랜 구성을 변경할 수 있으며, 중요한 변경사항은 최소 7일 전 공지합니다.
- 서버 점검, 천재지변, 기술적 장애 등 불가피한 사유로 서비스가 일시 중단될 수 있습니다.
- 운영자의 귀책이 아닌 사유로 인한 서비스 중단에 대해 운영자는 책임을 지지 않습니다.

---

### 7. 면책 및 책임 제한

- Easy-Easy는 AI를 활용한 문서 해석 서비스를 제공하며, AI의 특성상 해석 결과의 완전한 정확성을 보장하지 않습니다. 해석 결과를 법적·의료적·재무적 판단에 직접 활용하는 경우 발생하는 결과에 대해 운영자는 책임을 지지 않습니다.
- 이용자가 업로드하는 문서의 저작권·기밀성 등 관리 책임은 이용자 본인에게 있습니다.

---

### 8. 약관 변경

본 약관은 관련 법령 변경 또는 서비스 정책 변경에 따라 수정될 수 있습니다. 변경 시 서비스 내 공지 또는 이메일을 통해 안내드립니다.

---

### 9. 준거법 및 분쟁 해결

본 약관은 대한민국 법률에 따라 해석되며, 서비스 이용과 관련한 분쟁은 대한민국 법원을 관할로 합니다.
...
Easy-Easy PRO 구독은 **디지털 서비스(즉시 이용 개시)**의 특성상, 결제 완료 후에는 원칙적으로 환불이 제공되지 않습니다.

구독을 취소하더라도 이미 결제된 기간의 잔여분에 대한 부분 환불은 이루어지지 않으며, 해당 구독 기간이 종료될 때까지 PRO 혜택이 유지됩니다.

---

### 구독 취소 방법

구독 취소를 원하시면 아래 이메일로 요청해 주세요.

📧 **tryyrt42@gmail.com**

- 취소 요청 접수 후 다음 결제일 이전에 처리됩니다.
- 취소 후에는 현재 구독 기간 종료 시 FREE 플랜으로 자동 전환됩니다.
- 취소해도 남은 구독 기간 동안 PRO 혜택은 유지됩니다.

---

### 예외적 환불 처리

다음의 경우에는 예외적으로 환불 검토가 가능합니다:

- **서비스 제공 불가:** 운영자 귀책으로 인해 구독 기간 중 서비스가 연속 72시간 이상 제공되지 않은 경우
- **이중 결제:** 시스템 오류로 동일 건이 중복 결제된 경우
- **관련 법령에 따른 소비자 권리 행사:** 대한민국 전자상거래 등에서의 소비자보호에 관한 법률 등 관련 법령에서 인정하는 권리를 행사하는 경우

예외 환불 요청은 사유 발생일로부터 **7일 이내**에 이메일로 문의해 주세요.

---

### 결제 처리 주체

Easy-Easy의 결제는 **Lemon Squeezy(Stripe 자회사)**가 판매 주체(Merchant of Record)로서 처리합니다. 결제 영수증, 청구 관련 분쟁은 Lemon Squeezy 고객센터를 통해서도 처리될 수 있습니다.
...
응답은 영업일 기준 1~3일 이내에 드립니다.
""")


# --- 라우팅: ?page=privacy / terms / refund 면 해당 페이지만 띄우고 중단 ---
...
def _parse_dt(s):
    """timestamptz 문자열을 aware datetime 으로 파싱 (실패 시 None)."""
    if not s:
        return None
    from datetime import datetime
    txt = str(s).strip().replace("Z", "+00:00")
    for cand in (txt, txt.replace(" ", "T", 1)):
        try:
...
def _roll_forward_30(start, now):
    """start 기준 30일 주기로 now 를 넘어서는 다음 경계 반환."""
...
if st.session_state.get("user") is None:
    ...
    st.markdown(
        ...
        ">
            ...
            <span style="margin: 0 0.5rem; color: #475569;">|</span>
            <a href="?page=refund" target="_blank"
               style="color: #94a3b8; text-decoration: none;">
                환불정책
            </a>
        </div>
